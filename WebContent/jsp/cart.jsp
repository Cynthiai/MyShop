			...
			<div class="row">
							...
							<tbody>
								...
								<c:forEach items="${cart.items }" var="item">
									<tr class="active">
										...
										<td>
											<a href="javascript:void(0)" class="delete" onclick="deleteItem('${item.product.pid}')">删除</a>
										</td>
									</tr>
								</c:forEach>
						</tbody>
				...
				<div class="row">
					<c:if test="${empty cart.map }">
						<center><h1>购物车空空如也，快去添加商品！</h1></center>
					</c:if>
				</div>
			</div>
...
<script type="text/javascript">
	function deleteItem(pid){
		if(confirm("确定要删除吗？")){
			location.href="${pageContext.request.contextPath}/cart?method=removeFromCartItem&pid="+pid;
		}
	}
</script>
</html>